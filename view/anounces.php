...
<table class="table table-striped ">
  ...
  <?php foreach($events as $key  => $oneEvent){ ?>
    <tr>
      ...
      <?php if(array_key_exists('offering', $oneEvent)){ ?>
      <td><?=$oneEvent['offering']?></td>
      <?php } ?>
      <td>editar</td>
      <td><a type=button href="index.php?action=home&date=<?=$oneEvent['date']?>&id=<?=$oneEvent['id_event']?>" class="btn btn-success"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
